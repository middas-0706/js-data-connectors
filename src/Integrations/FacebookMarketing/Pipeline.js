...
startImportProcess() {
  ...
  let accountsIds = this.config.AccoundIDs.value.split(/[,;]\s*/);
    ...
    for (let accountId of accountsIds) {
      ...
      for(var nodeName in timeSeriesNodes) {
        ...
        if( !data.length ) {      
          ...
          if( daysShift == 0) {
            this.config.logMessage(`ℹ️ No records have been fetched for account ${accountId}`);
          }

        } else {

          this.config.logMessage(`${data.length} rows were fetched for ${accountId}/${nodeName}/${Utilities.formatDate(startDate, "UTC", "yyyy-MM-dd")}`);
