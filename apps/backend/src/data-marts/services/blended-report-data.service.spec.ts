...
function makeBlendableSchema(
  blendedFieldNames: string[] = [],
  /** What the producer reports as hidden rather than lost — own paths and joined unified names. */
  hiddenFieldNames: string[] = [],
  hiddenBlendedFieldNames: readonly string[] = []
): BlendableSchemaDto {
  return {
    hiddenFieldNames,
    nativeFields: [],
    ...
    blendedFields: blendedFieldNames.map((name, i) => {
      ...
      field.type = 'STRING';
      field.isHidden = hiddenBlendedFieldNames.includes(name);
...
describe('BlendedReportDataService', () => {
  ...
  describe('resolveBlendingDecision', () => {
    ...
    describe('resolveBlendingDecision — orphaned column references', () => {
      ...
      it.each([
        [
          'hidden on the joined Data Mart itself',
          // Such a field never reaches `blendedFields` at all — the producer drops it before the
          // DTOs are built — so the run sees a name nothing resolves, exactly like a lost column.
          () => makeBlendableSchema(['blended_field'], ['alias__hidden_field']),
        ],
        [
          'hidden per join, in the Report Fields tab',
          // This one DOES reach `blendedFields`, carrying `isHidden` — a different route to the
          // same dead end, and the reason the producer reports both under one list.
          () =>
            makeBlendableSchema(
              ['blended_field', 'alias__hidden_field'],
              ['alias__hidden_field'],
              ['alias__hidden_field']
            ),
        ],
      ])('calls a joined column %s hidden, not disconnected', async (_label, buildSchema) => {
        // The set this check reads used to come from the main Data Mart's own schema, which knows
        // nothing about joined names — so the run kept saying "restore the schema" while the
        // picker, the save and the SQL preview all called the same column hidden.
        const report = makeReport({
          columnConfig: ['date', 'alias__hidden_field', 'blended_field'],
        });
        report.dataMart.schema = makeMainSchema([nativeField('date')]);
        blendableSchemaService.computeBlendableSchema.mockResolvedValue(buildSchema());
        mockChains();
        ...
        expect(error.message).toContain('Hidden columns: "alias__hidden_field".');
        expect(error.message).toContain('or ask your analyst to show them in reports again.');
        expect(error.message).not.toContain('Disconnected columns');
        expect(error.errorDetails).toMatchObject({
          unknownColumns: ['alias__hidden_field'],
          hiddenColumns: ['alias__hidden_field'],
        });
        expect(blendedQueryBuilderFacade.buildBlendedQuery).not.toHaveBeenCalled();
      });

      it('still calls a joined column disconnected when the join or the field is gone', async () => {
        // Nothing hid it: the name resolves to no blended field and appears in no hidden list,
        // which is what a removed relationship or a deleted field looks like from here.
        const report = makeReport({ columnConfig: ['date', 'alias__gone', 'blended_field'] });
        report.dataMart.schema = makeMainSchema([nativeField('date')]);
        blendableSchemaService.computeBlendableSchema.mockResolvedValue(
          makeBlendableSchema(['blended_field'])
        );
        ...
        expect(error.message).toContain('Disconnected columns: "alias__gone".');
        expect(error.message).toContain('or contact your analyst to restore the schema.');
        expect(error.errorDetails).not.toHaveProperty('hiddenColumns');
      });
