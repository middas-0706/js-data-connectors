...
- When a data mart defines the quantity being asked for as a Calculated Field whose "calculated" level is "metric", select that field by name instead of rebuilding it from the columns it is computed from. OWOX then computes it over every matching row; on such a field an empty allowedAggregations means the value is already computed, not that the field is unusable.
