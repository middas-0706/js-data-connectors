...
function importNewData() {
  ...
  const connector = new OWOX.CriteoAdsConnector(config.setParametersValues(properties));
...
function updateFieldsSheet() {
  ...
  config.updateFieldsSheet(
    new OWOX.CriteoAdsConnector(config.setParametersValues({
      "ClientId": "undefined", 
...
function manageCredentials(credentials) {
  ...
  if (!credentials) {
    ...
    const config = new OWOX.GoogleSheetsConfig(CONFIG_RANGE);
    const connector = new OWOX.CriteoAdsConnector(config);
