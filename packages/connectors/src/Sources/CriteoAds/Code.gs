...
function manualBackfill() {
  const config = new OWOX.GoogleSheetsConfig(CONFIG_RANGE);
  const source = new OWOX.CriteoAdsSource(config.setParametersValues(
...
function updateFieldsSheet() {
  ...
  config.updateFieldsSheet(
    new OWOX.CriteoAdsSource(config.setParametersValues({
      "ClientId": "undefined", 
...
function manageCredentials(credentials) {
  ...
  if (!credentials) {
    ...
    const config = new OWOX.GoogleSheetsConfig(CONFIG_RANGE);
      const source = new OWOX.CriteoAdsSource(config);
