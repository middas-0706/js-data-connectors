...
| `Please reduce the amount of data you're asking for, then retry your request` or request timeout errors | The request asks Meta for too much data. | Reduce the date range, fields, or breakdowns. Insights Data Marts select `conversions` and `conversion_values` by default. Both cost Meta a lot to compute, so clear them first if you do not report on them. Then lower **API Page Limit** in **Advanced settings** and rerun. |
