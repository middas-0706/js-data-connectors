...
describe('SnowflakeBlendedQueryBuilder', () => {
  it.each(['ARRAY', 'OBJECT', 'VARIANT'])(
    'keeps native %s opaque in existing report schemas',
    type => {
      expect(parseSnowflakeFieldType(type)).toBe(SnowflakeFieldType.VARIANT);
    }
  );

  // Dialect contract for an explicitly supplied array descriptor. Current Snowflake
  // schemas remain VARIANT, so these fixtures do not claim native ARRAY classification.
  describe('JSON transport for a supplied array descriptor', () => {
    const arrayField = {
