...
function importNewData() {
  ...
  const connector = new OWOX.BingAdsConnector(config.setParametersValues(properties));
...
function updateFieldsSheet() {
  ...
  config.updateFieldsSheet(
    new OWOX.BingAdsConnector(config.setParametersValues({
...
function manageCredentials(credentials) {
  ...
  if (!credentials) {
    ...
    const config = new OWOX.GoogleSheetsConfig(CONFIG_RANGE);
    const connector = new OWOX.BingAdsConnector(config);
